...
- **Async** — git/lang/gh refresh in a ~3 KB subshell. First paint never blocks (`async_repaint=1`); every prompt kicks a fresh git refresh, so editor-only edits show up on the next prompt. `async_timeout` (default `5`s) kills runaway bg work.
