...
- **Adaptive git** — git status runs inline by default, so a normal repo reflects on the very prompt it changed (no stale paint). The first compute per dir is timed; only a repo slower than `async_threshold` ms (default `80`) flips to the serve-cache-then-bg-refresh path. The fast/slow verdict is cached per dir (cleared by `damin_reset_cache`). `async_threshold 0` forces async everywhere; `async_git 0` (or `async_repaint 0`) forces inline everywhere.
- **Async** — lang/gh and slow-repo git refresh in a ~3 KB subshell. The async git path serves the cache, bg-refreshes (`async_repaint=1`), and signals a repaint when done (coalesced — one in-flight worker per key). `async_timeout` (default `5`s) kills runaway bg work.
