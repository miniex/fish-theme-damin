...
- **Bundled `.theme` files for `fish_config`** — four themes (`Damin Mocha` / `Frappe` / `Macchiato` / `Latte`) shipped under `themes/`. `damin_install_themes` symlinks them into `~/.config/fish/themes/` so they appear in `fish_config theme show` and can be applied via fish's standard theming flow
...
- **Named exit codes** — opt-in `theme_damin_status_names` rewrites the failure indicator: `126` → `noexec`, `127` → `not-found`, and `128+N` → signal names (`SIGINT` / `SIGKILL` / `SIGTERM` / `SIGSEGV` / `SIGPIPE` …) via fish's `fish_status_to_signal` builtin. Codes without a mapping keep the raw number. Toggle: `theme_damin_status_names` (default 0)
