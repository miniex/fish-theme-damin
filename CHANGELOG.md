...
- `tools/test.sh` — POSIX-shell harness that runs `_damin_git_compute` against fixture git repos (clean / dirty mixes / detached HEAD / stash / upstream ahead-behind / no-upstream rev-list fallback / rebase / merge / `theme_damin_show_git_op=0` gate), and `_damin_k8s_compute` against fixture kubeconfigs (single context, multiple contexts, no namespace, current-context before contexts, quoted values), and `_damin_env_render` for direnv/nix combinations. Covers every prior parser regression so the next porcelain-v2 / kubeconfig / env edge case fails loudly
